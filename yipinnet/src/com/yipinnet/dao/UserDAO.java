...
public class UserDAO {
	...
	private static List<User> findBySQL(String sql){
		...
		try{
			while(rs.next()){
				...
				user.setPriviledge(rs.getString(5));
				list.add(user);
			}
			return list;
	...
	public static List<User> u_login(User user){
		String sql = "select * from user where 1=1 and priviledge='" + "0" + "'";
	...
	public static List<User> login(User user){
		String sql = "select * from user where 1=1 and priviledge='" + "1" + "'";
		if(!user.getUsername().equals("") && !user.getPassword().equals("")){
			sql += " and username='" + user.getUsername() + "'" + "and password='" + user.getPassword()+"'";
		}else{
			List<User> list = new ArrayList<User>();
	...
	public static int save(User user){
		...
		sbf.append("insert into user(username,password,email,priviledge) values(");
	    sbf.append("'" + user.getUsername() + "',");
	    sbf.append("'" + user.getPassword() + "',");
	    sbf.append("'" + user.getEmail() + "',");
	    sbf.append("'" + user.getPriviledge() + "')");
	...
	public static int update(User user){
		...
		+ user.getEmail() + "',priviledge='" + user.getPriviledge() +
